...
try {
    ...
    if ($ttd) {
        ...
        if ($resTtd->num_rows > 0) {
            $updTtd = $koneksi->prepare("UPDATE observasi_ttd SET ttd_penilai_oss=?, tanggal_ttd_oss=? WHERE id_siswa_ttd_oss=?");
            $updTtd->bind_param("ssi", $ttd, $tgl, $id_siswa);
            $updTtd->execute();
            $updTtd->close();
        } else {
            $insTtd = $koneksi->prepare("INSERT INTO observasi_ttd (id_siswa_ttd_oss, tanggal_ttd_oss, ttd_penilai_oss) VALUES (?,?)");
            $insTtd->bind_param("iss", $id_siswa, $tgl, $ttd);
            $insTtd->execute();
            $insTtd->close();
        }
        $cekTtd->close();
