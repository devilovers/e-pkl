...
<?php if (empty($data_du_list)): ?>
    <p style="text-align:center; color:#777;">Tidak ada data siswa ini.</p>
<?php else: ?>
...
function openModal(data) {
    ...
    fetch(`get_observasi.php?id_jur=${data.id_jur}&id_siswa=${data.datawal_id_siswa}`)
        ...
        .then(rows => {
            ...
            // tambahkan 2 canvas untuk tanda tangan
            let ttdHtml = `
                <div style="display:flex;justify-content:space-between;margin-top:20px;">
                    <div style="text-align:center;width:45%;">
                        <p><b>Tanda Tangan Pembimbing</b></p>
                        <canvas id="canvasPembimbing" width="250" height="120" style="border:1px solid #000;"></canvas>
                    </div>
                    <div style="text-align:center;width:45%;">
                        <p><b>Tanda Tangan Penilai</b></p>
                        <canvas id="canvasPenilai" width="250" height="120" style="border:1px solid #000;"></canvas>
                    </div>
                </div>
                <div style="margin-top:20px;text-align:center;">
                    <button id="btnCetak" class="btn btn-primary">Cetak Lembar Observasi PKL</button>
                </div>
            `;

            body.innerHTML = siswaHtml + tableHtml + ttdHtml;
            modal.style.display = 'flex';

            // tampilkan tanda tangan lama
            initSignature("canvasPembimbing", data.ttd_pembimbing_oss || null);
            initSignature("canvasPenilai", data.ttd_penilai_oss || null);

            // tombol cetak -> arahkan ke file cetak mpdf
            document.getElementById('btnCetak').addEventListener('click', function () {
                window.open(
                    `cetak_observasi.php?id_jur=${data.id_jur}&id_siswa=${data.datawal_id_siswa}`,
                    '_blank'
                );
            });
        })
...
function initSignature(canvasId, existingTtd = null) {
    let canvas = document.getElementById(canvasId);
    if (!canvas) return; // kalau canvas tidak ada, hentikan
    ...
    // Kalau ada tanda tangan lama (base64), render ke canvas
    if (existingTtd) {
        let img = new Image();
        img.onload = () => {
    ...
    // Event menggambar manual (jika ingin aktifkan tanda tangan langsung di modal)
    let drawing = false;
